...
RUN cd /pdal && mkdir build && cd build && cmake -G Ninja .. && sed -i 's/CMAKE_BUILD_TYPE:STRING=/CMAKE_BUILD_TYPE:STRING=Release/' ./CMakeCache.txt && echo 'WITH_LASZIP:BOOL=ON' >> ./CMakeCache.txt && ninja && ninja install && cd /
